...
static void recursive_lookup(woart_node *n, unsigned long num, 
	...
	switch (n->type) {
		case NODE4:
			for (i = 0; i < ((((woart_node4*)n)->slot[i].i_ptr != -1) && i < 4); i++) {
				recursive_lookup(((woart_node4*)n)->children[((woart_node4*)n)->slot[i].i_ptr],
		...
		case NODE48:
			...
			for (i = 0; i < 256; i++) {
				...
				if (!idx) continue;
				count48++;
				recursive_lookup(((woart_node48*)n)->children[idx - 1], num,
						search_count, buf);
		...
		case NODE256:
			for (i=0; i < 256; i++) {
				if (!((woart_node256*)n)->children[i]) continue;
				recursive_lookup(((woart_node256*)n)->children[i], num,
						search_count, buf);
				if (*search_count == num)
					break;
			}
