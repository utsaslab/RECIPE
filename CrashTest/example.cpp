...
namespace Dummy {
    ...
    void clflush(char *data, int len, bool fence)
    {
        if (fence)
            mfence();
    ...
    if (index_type == TYPE_ART) {
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    Key *key = key->make_leaf((char *)init_keys[i]->fkey, init_keys[i]->key_len, init_keys[i]->value);
                    clflush((char *)key, sizeof(Key) + key->key_len, true);
                    if (!(mTrie.insert(key))) {
            ...
            auto func = [&]() {
                ...
                for (uint64_t i = start_key; i < end_key; i++) {
                    if (ops[i] == OP_INSERT) {
                        ...
                        if (it.IsEnd() == true) {
                            std::cout << "[BWTREE] Iterator reaches the end\n";
                        }
                    }
                }
                t->UnregisterThread(thread_id);
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        ...
                        if (*ret != keys[i]->value) {
                            printf("[FASTFAIR] search key = %lu, search value = %lu\n", keys[i]->value, *ret);
                            exit(1);
                        }
                    } else if (ops[i] == OP_SCAN) {
                        uint64_t buf[200];
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    ...
                    Dummy::mfence();
                    Dummy::clflush((char *)dummy[i].key, init_keys[i]->key_len, false);
                    Dummy::mfence();
                    Dummy::clflush((char *)&dummy[i], sizeof(uentry), false);
                    Dummy::mfence();
                }
            });
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        ...
                        Dummy::mfence();
                        Dummy::clflush((char *)dummy[i + LOAD_SIZE].key, keys[i]->key_len, false);
                        Dummy::mfence();
                        Dummy::clflush((char *)&dummy[i], sizeof(uentry), false);
                        Dummy::mfence();
                    } else if (ops[i] == OP_READ) {
    ...
    if (index_type == TYPE_ART) {
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    Key *key = key->make_leaf(init_keys[i], sizeof(uint64_t), init_keys[i]);
                    clflush((char *)key, sizeof(Key) + sizeof(uint64_t), true);
                    if (!(mTrie.insert(key))) {
                        fprintf(stderr, "[HOT] load insert fail\n");
                        exit(1);
                    }
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        ...
                        auto val = table->Get(keys[i]);
                        if (val == NONE) {
                            std::cout << "[CCEH] wrong key read: " << *(uint64_t *)val << " expected: " << keys[i] << std::endl;
                            exit(1);
                        }
                    } else if (ops[i] == OP_SCAN) {
                    }
...
int main(int argc, char **argv) {
    ...
    if (argc != 7) {
#endif
        std::cout << "Usage: ./example [index type] [ycsb workload type] [key distribution] [access pattern] [number of threads]\n";
